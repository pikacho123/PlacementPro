...
<div class="content">
    ...
    <table>
        ...
        <tbody>
            ...
            try {
                Connection con = ConnectionProvider.getCon();
                PreparedStatement ps = con.prepareStatement("SELECT s.student_id, s.first_name, s.last_name, MAX(c.company_name) AS company_name, MAX(s.placement_date_time) AS placement_date_time FROM student s INNER JOIN application a ON s.user_id = a.user_id INNER JOIN Placement_drive p ON a.drive_id = p.drive_id INNER JOIN Company_master c ON p.company_id = c.company_id WHERE a.status = 'accepted' GROUP BY s.student_id, s.first_name, s.last_name");
                ResultSet rs = ps.executeQuery();
                while (rs.next()) {
            %>
            <tr>
